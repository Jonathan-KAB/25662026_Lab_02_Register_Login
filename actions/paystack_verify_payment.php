...
try {
    ...
    try {
        // Generate invoice number (numeric only for database)
        $invoice_no = (int)(date('Ymd') . substr(uniqid(), -4));
        $order_date = date('Y-m-d');
        
        // Create order in database
        $order_id = create_order_ctr($customer_id, $invoice_no);
        ...
        // Update order status to 'Paid' since payment is verified
        update_order_status_ctr($order_id, 'Paid');
        ...
        // Log activity
        error_log("User activity: Customer $customer_id completed payment via Paystack - Invoice: $invoice_no, Amount: GHS $total_amount, Reference: $reference");
        
        // Format invoice for display
        $invoice_display = 'INV-' . $invoice_no;
